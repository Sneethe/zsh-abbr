...
abbr() {
...

  {
    ...
    version="zsh-abbr version 6.1.0"
    ...
    _abbr:erase() {
      ...
      if ! (( ${#abbreviations_sets} )); then
        _abbr:util_error "abbr erase: No${type:+ $type}${scope:+ $scope} abbreviation \`${(Q)${(q)abbreviation}}\` found"
        ...
        _abbr:util_log_unless_quiet "$success_color$verb_phrase$reset_color $(_abbr:util_set_to_typed_scope $abbreviations_sets) \`${(Q)${(q)abbreviation}}\`"
      else
